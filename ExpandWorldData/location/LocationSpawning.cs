...
public class LocationObjectDataAndSwap
{
    ...
    static void Postfix(ZoneSystem.ZoneLocation location, int seed, Vector3 pos, Quaternion rot, ZoneSystem.SpawnMode mode, List<GameObject> spawnedGhostObjects)
    {
        ...
        if (LocationExtra.TryGetData(location, out var data))
        {
            ...
            // Restored as Jere is right and terrain modification must run here, as Ghost mode is how dedicated servers permanently generate the world.
        ...
        {
            // Guaranteed to run even if a blueprint or custom object crashes preventing m_ghostInit from becoming permanently stuck to true which was likely the issue that ashenius was experiencing.
            if (ghostStarted)
            {
