...
{
    static bool Prefix(ZoneSystem.ZoneLocation location, ZoneSystem.SpawnMode mode, ref Vector3 pos, ref int seed, List<GameObject> spawnedGhostObjects, object[] __args)
    {
        if (mode != ZoneSystem.SpawnMode.Client)
        {
            ...
            if (LocationExtra.TryGetData(location, out var data))
            {
                ...
                if (data.randomCardinal)
                {
                    ...
                    int directionMultiplier = UnityEngine.Random.Range(0, 4);
                    __args[3] = Quaternion.Euler(0f, directionMultiplier * 90f, 0f);//ref Quaternion rot was causing a crash. bug? Was causing the world having vanilla biomes only.
    ...
    static void Postfix(ZoneSystem.ZoneLocation location, int seed, Vector3 pos, Quaternion rot, ZoneSystem.SpawnMode mode, List<GameObject> spawnedGhostObjects)
    {
        ...
        if (LocationExtra.TryGetData(location, out var data))
        {
            //cardinal in postfix for bps and custom stuff, since I had to use the __args[3]
            if (data.randomCardinal)
            {
                UnityEngine.Random.State previousState = UnityEngine.Random.state;
                UnityEngine.Random.InitState(seed);
                rot = Quaternion.Euler(0f, UnityEngine.Random.Range(0, 4) * 90f, 0f);
                UnityEngine.Random.state = previousState;
            }
