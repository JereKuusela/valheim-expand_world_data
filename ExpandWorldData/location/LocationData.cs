...

{
    ...
    // Search-only anchor groups, format "group,distance". The location must spawn within <distance> of a location

    // that ADVERTISES <group> (a host declaring groupMax: <group>), but it never advertises the group itself, so

    // satellites cluster around hosts without seeding each other. Distance is optional and falls back to

    // maxDistanceFromSimilar. -Nick

    [DefaultValue(null)]

    public string[]? anchors = null;
